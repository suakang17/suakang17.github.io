...
일반적으로 더 이상 사용하지 않는 객체 변수에 null을 할당하면 GC(Garbage Collector)가 해당 메모리를 수거해 갈 것이라고 기대한다. 하지만 자바의 메모리 관리 구조, 특히 뷰(View)와 참조(Reference)의 개념을 명확히 이해하지 못하면 트래픽이 몰릴 때 OOM(Out Of Memory) 상황을 맞이할 수 있다.
...
메모리 관리를 위해 WeakHashMap을 캐시로 사용하는 경우가 종종 있다. 하지만 이 자료구조 역시 정확한 동작 방식을 이해하고 사용해야 한다.

WeakHashMap은 Key에 대한 참조가 Weak인 맵이다. 즉, Key 객체가 더 이상 다른 곳에서 참조되지 않을 때 해당 엔트리를 삭제한다. Value의 메모리 점유 상태와는 무관하다.

만약 캐시의 Key로 String 리터럴이나 Integer 같은 상수를 사용한다면 주의가 필요하다. 자바의 String Pool 등으로 인해 해당 객체들은 프로그램 종료 시까지 강한 참조(Strong Reference)로 남아있을 가능성이 높기 때문이다. Key가 수거되지 않으면 Value에 저장된 데이터 역시 영원히 메모리에 남게 되어 누수의 원인이 될 수 있다.

따라서 범용적인 데이터 캐싱이 목적이라면 Caffeine이나 Ehcache와 같이 LRU(Least Recently Used) 알고리즘 등을 지원하는 검증된 캐시 라이브러리를 사용하는 것이 적합하다.

## 요약

List.subList는 원본 리스트의 뷰(View)이므로 원본 객체에 대한 참조를 유지한다.
